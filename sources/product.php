<?php  
	...
	{
		/* Lấy sản phẩm detail */
		$row_detail = $d->rawQueryOne("select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giamoi, gia, giasi from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		/* Lấy cấp 1 */
		$pro_list = $d->rawQueryOne("select id, ten$lang,gia,  tenkhongdauvi, tenkhongdauen from #_product_list where id = ? and type = ? and hienthi > 0 limit 0,1",array($row_detail['id_list'],$type));
		...
		$sql = "select photo, ten$lang,phobien,luotxem, noidung$lang, mota$lang, tenkhongdauvi, tenkhongdauen, giamoi, gia, giakm, id from #_product where $where order by stt,id desc $limit";
	...
	{
		/* Lấy cấp 1 detail */
		$pro_list = $d->rawQueryOne("select id, ten$lang,gia, tenkhongdauvi, tenkhongdauen, type, photo, options from #_product_list where id = ? and type = ? limit 0,1",array($idl,$type));
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select photo,gia, ten$lang, mota$lang, noidung$lang, tenkhongdauvi, tenkhongdauen, giamoi, gia, giakm, id from #_product where $where order by stt,id desc $limit";
	...
	else if($idc!='')
	{
		...
		/* Lấy cấp 1 */
		$pro_list = $d->rawQueryOne("select id, ten$lang,gia, tenkhongdauvi, tenkhongdauen from #_product_list where id = ? and type = ? limit 0,1",array($pro_cat['id_list'],$type));
	...
	else if($idi!='')
	{
		...
		/* Lấy cấp 1 */
		$pro_list = $d->rawQueryOne("select id, ten$lang,gia, tenkhongdauvi, tenkhongdauen from #_product_list where id = ? and type = ? limit 0,1",array($pro_item['id_list'],$type));
	...
	else if($ids!='')
	{
		...
		/* Lấy cấp 1 */
		$pro_list = $d->rawQueryOne("select id, ten$lang,gia, tenkhongdauvi, tenkhongdauen from #_product_list where id = ? and type = ? limit 0,1",array($pro_sub['id_list'],$type));
	...
	else
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select photo, ten$lang,giasi ,mota$lang,luotxem, tenkhongdauvi, tenkhongdauen, giamoi, gia, giakm, id from #_product where $where order by stt,id desc $limit";
		$product = $d->rawQuery($sql,$params);
		$sqlNum = "select count(*) as 'num' from #_product where $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,$params);
		$total = $count['num'];
