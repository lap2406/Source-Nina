...
<?php if ($type == 'video-fotorama') {
	...
	if (count($videonb)) { ?>
		<!-- Video Fotorama -->
		<div id="fotorama-videos" data-width="100%" data-height="500" data-thumbmargin="15" data-height="100" data-fit="cover" data-thumbwidth="413" data-thumbheight="246" data-allowfullscreen="true" data-nav="thumbs">
...
<?php if ($type == 'video-custom') {
	$videonb = $d->rawQuery("select link_video, id, ten$lang from #_photo where type = ? and act <> ? and noibat > 0 and hienthi > 0 order by stt, id desc", array('video', 'photo_static'));

	if (count($videonb)) { ?>
		<div class="video-iframe">
			<iframe width="100%" height="100%" id="iframe-video" src="https://www.youtube.com/embed/<?= $func->getYoutubeIdFromUrl($videonb[0]['link_video']) ?>" frameborder="0"></iframe>
		</div>
		<div class="slick-video ">
			<?php foreach ($videonb as $k => $v) { ?>
				<div class=" mt-2 ">
					<div class="video-clip p-3">
						<a data-link="https://www.youtube.com/embed/<?= $func->getYoutubeIdFromUrl($v['link_video']) ?>">
							<p class="pic-video scale-img"><img onerror="this.src='<?= THUMBS ?>/417x246x1/assets/images/noimage.png';" src=" https://img.youtube.com/vi/<?= $func->getYoutube($v['link_video']) ?>/0.jpg" alt="<?= $v['ten' . $lang] ?>" /></p>
						</a>
					</div>
				</div>
			<?php } ?>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				$(".video-clip a").click(function() {
					var linkvideo = $(this).attr('data-link');
					$("#iframe-video").attr('src', linkvideo);
				});
				$(".slick-video").slick({
					autoplay: true,
					arrows: false,
...
<?php if ($type == 'video-select') {
	...
	if (count($videonb)) { ?>
		...
		<div class="video-main">
			<iframe width="100%" height="380px" src="//www.youtube.com/embed/<?= $func->getYoutube($videonb[0]['link_video']) ?>" frameborder="0" allowfullscreen></iframe>
		</div>
