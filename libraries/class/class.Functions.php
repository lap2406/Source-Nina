...
class Functions
{
	...
	function showProduct($v, $options = array(), $k)
	{
		if (!isset($options['class'])) {
			$options['class'] = 'col-2-5 mb-3	';
		}
		global $config_url, $lang, $sluglang;
		$error = "this.src='images/no_photo.png'";
		echo '<div class="' . $options['class'] . ' " data-wow-offset="0" data-wow-duration="1" data-wow-delay="' . (0.2 * $k) . 's" data-id="' . $v['id'] . '" data-name="' . $v['ten' . $lang] . '">';
		echo '<div class="product-items product-items-'.$v['id'].'">';
		echo '<div class="product-img" id="product-img">';
		echo '<div class="images">';
		echo '<a title="' . $v['ten' . $lang] . '" href="' . $v[$sluglang] . '">';
		echo '<img onerror="' . $error . '" src="' . THUMBS . '/230x250x2/' . UPLOAD_PRODUCT_L . $v['photo'] . '" alt="' . $v['ten' . $lang] . '">';
		echo '</a>';
		...
		echo '<h3><a title="' . $v['ten' . $lang] . '" href="' . $v[$sluglang] . '">' . $v['ten' . $lang] . '</a></h3>';
		echo '<div class="product-prices">';
		if (isset($v['gia']) && $v['gia'] > 0) {
			echo '<span>Giá lẻ: <b>' . $this->format_money($v['gia']) . '</b></span>';

		}
		if (isset($v['giasi']) && $v['giasi'] > 0) {
			echo '<span>Giá sỉ: <b>' . $this->format_money($v['giasi']) . '</b></span>';

		}
		if ($v['gia'] == 0 && $v['gia'] == 0) {
			echo '<span class="text-center">Giá lẻ: <strong>Liên hệ</strong></span>';
		}
		if ($v['giasi'] == 0 && $v['giasi'] == 0) {
			echo '<span class="text-center">Giá sỉ: <strong>Liên hệ</strong></span>';
		}

		echo '</div>';
		echo ' <div class="pronew-addcart">';
		echo '<button class="transition addcart"  id="add-cart" data-name="' . $v['ten' . $lang] . '" data-id="' . $v['id'] . '" data-action="addnow"> ';
		echo '<span><p>Chọn mua </p></span>';
		echo '</button>';
		echo ' </div> ';
		// echo'<p>'.$v['mota' . $lang].'</p>';
		echo '</div>';
		echo '</div>';
		echo '</div>';
	}
	function showProductOld($v, $options = array(), $k)
	{
	...
	/* Format money */
	public function format_money($price = 0, $unit = ' ₫', $html = false)
	{
		$str = '';
